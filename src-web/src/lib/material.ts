import { LoadingManager, Material, MeshBasicMaterial, Texture, TextureLoader } from "three";
...
let cache: Map<string, {frames: AnimationFrame[], aspectRatio: number, animationLength: number}> = new Map()
...
export class EmoteMaterial extends MeshBasicMaterial {
    ...
    constructor(channel: string, emote: ChannelEmote, onLoad?: (mat: EmoteMaterial) => void | Promise<void>) {
        ...
        fetch(`https://overlay-api.juliapixel.com/emote/${channel}/${emote.name}`).then(async (resp) => {
            ...
            for (let i = 0; i < emoteInfo.frame_count; i++) {
                ...
                textureLoader.loadAsync(`https://overlay-api.juliapixel.com/emote/${channel}/${emote.name}/${i}.webp`).then((tex) => {
                    if (i === 0) {
                        this.map = tex;
                        this.aspectRatio = tex.source.data.naturalWidth / tex.source.data.naturalHeight;
                    }

                    tex.colorSpace = "srgb"

                    this.frames[i] = { texture: tex, delay: emoteInfo.frame_delays[i] };
                    ...
                    if (processedFrameCount === emoteInfo.frame_count) {
                        cache.set(
                            ...
                            { frames: this.frames, aspectRatio: this.aspectRatio, animationLength: this.animationLength }
                        )
                        if (onLoad) { onLoad(this) }
                    }
                })
